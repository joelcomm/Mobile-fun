...
export class DogSprite extends Phaser.GameObjects.Container {
  ...
  private belly!: Phaser.GameObjects.Rectangle;
  private head!: Phaser.GameObjects.Rectangle;
  private earL!: Phaser.GameObjects.Rectangle;
  private earR!: Phaser.GameObjects.Rectangle;
  private snout!: Phaser.GameObjects.Rectangle;
  ...
  constructor(scene: Phaser.Scene, data: DogData) {
    ...
    this.setInteractive(
      new Phaser.Geom.Rectangle(-20, -22, 40, 40),
  ...
  playMoodBubble(icon: string): void {
    const sign = this.scaleX < 0 ? -1 : 1;
