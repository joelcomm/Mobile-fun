...
export class YardScene extends Phaser.Scene {
    ...
    refreshPawPrints(delta) {
        this.pawTimer += delta;
        if (this.pawTimer < 90)
            return;
        this.pawTimer = 0;
        for (const [, sprite] of this.sprites) {
            if (sprite.dogData.animState !== "zoomies")
                continue;
    ...
    refreshKennelVisual(force = false) {
        ...
        if (fenceKey !== this.shownFenceKey)
            this.rebuildFences(fenceKey);
        const gateKey = level >= 50 ? "gate_gold" : level >= 20 ? "gate_iron" : "gate_wood";
        if (gateKey !== this.shownGateKey)
            this.rebuildGate(gateKey);
    ...
    rebuildFences(key) {
        for (const f of this.fenceSprites)
            f.destroy();
        this.fenceSprites = [];
        ...
        for (let i = 0; i < hCount; i++) {
            ...
            if (i >= midIdx - gateHalfTiles && i <= midIdx + gateHalfTiles - 1)
                continue;
    ...
    rebuildGate(gateKey) {
        ...
        this.gateSprite = this.add.image(YARD.x + YARD.width / 2, YARD.y + YARD.height + 8, gateKey);
        this.gateSprite.setDepth(-3);
        this.shownGateKey = gateKey;
    }
    /** Reveal/hide milestone decor items based on the current kennel level. */
    refreshMilestoneDecor(level) {
        for (const d of this.milestoneDecor)
            d.destroy();
        this.milestoneDecor = [];
    ...
    refreshBiome() {
        ...
        if (biome.id === this.shownBiomeKey)
            return;
    ...
    refreshKennelHouse() {
        ...
        if (tier === this.shownHouseTier && this.kennelHouse)
            return;
        if (this.kennelHouse)
            this.kennelHouse.destroy();
        this.kennelHouse = this.add.image(YARD.x + 36, YARD.y + 30, `kennel_house_${tier}`);
...
function blendColors(a, b, aWeight = 0.7) {
    const bw = 1 - aWeight;
