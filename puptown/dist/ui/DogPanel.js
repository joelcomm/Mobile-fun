...
import { BREED_LABELS, CENTER_DOG_CAP_PER_TIER, CENTER_JOY_BONUS_PER_TIER, CENTER_TIER_LABELS, LV_UP_BASE, LV_UP_MULT, MAX_CENTER_TIER, NAMING_COST_JOY, ROLE_INFO, GAME_WIDTH, } from "../config.js";
...
export class DogPanel extends Panel {
    ...
    refresh() {
        ...
        y += 58;
        this.drawTierUpgradeRow(y);
        y += 58;
        if (Game.instance().centers.canBuyMore()) {
    ...
    drawTierUpgradeRow(y) {
        ...
        bg.setOrigin(0.5);
        if (!cost) {
            const info = scene.add.text(18, y + 6, `${tierLabel.toUpperCase()} TIER (MAX)\nThis center is at full Tier ${MAX_CENTER_TIER}.`, {
                fontFamily: "monospace",
                fontSize: "12px",
                color: "#2a2a3e",
                lineSpacing: 3,
        ...
        const adoptionsOk = adoptionsHave >= adoptionsNeed;
        const canAfford = game.resources.canAfford({ joy: cost.joy });
        ...
        const info = scene.add.text(18, y + 6, `UPGRADE TIER (${tier}/${MAX_CENTER_TIER})\n${msg}`, {
            fontFamily: "monospace",
            fontSize: "12px",
            color: "#2a2a3e",
            lineSpacing: 3,
        });
        const color = locked ? 0xaaaaaa : canAfford ? 0xffd86b : 0xff9ac1;
        const btn = this.makeButton(GAME_WIDTH - 52, y + 26, 80, 40, `TIER ${tier + 1}\n☀️${formatNumber(cost.joy)}`, color);
        btn.bg.on("pointerdown", () => {
            if (locked) {
                this.flash(btn.bg, 0xff5a7e);
                return;
            }
            if (Game.instance().handleUpgradeCenterTier(center.id) !== null) {
                this.refresh();
            }
            else {
                this.flash(btn.bg, 0xff5a7e);
