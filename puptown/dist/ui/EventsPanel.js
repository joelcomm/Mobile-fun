...
export class EventsPanel extends Panel {
    ...
    refresh() {
        ...
        let y = Panel.TOP + 28;
        // Active status card.
        const activeBg = this.scene.add.rectangle(GAME_WIDTH / 2, y + 22, GAME_WIDTH - 32, 44, active ? 0xffd86b : 0xe8e0cf);
        ...
        for (const ev of RANDOM_EVENTS) {
            ...
            row.setOrigin(0.5);
            const eText = this.scene.add.text(22, y + 4, `${ev.icon} ${ev.name}\n${ev.description} (${ev.durationMs / 1000}s)`, {
                fontFamily: "monospace",
                fontSize: "11px",
                color: "#2a2a3e",
                lineSpacing: 2,
